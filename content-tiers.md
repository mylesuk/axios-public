...
- **Source:** a mature insight, a finished article (derivative via `q-ship`), or a clean self-contained observation.
...
- **Format:** longform markdown (`60-Catalog/Articles/`), produced through `/draft` (or `q-draft` handoff).
...
Not every piece uses this framing. Only those where the historical anchor is doing real load-bearing work — not decoration. When a `q-draft` or `q-ship` handoff sets `series: truth-over-time`, the Truth-over-time block is required in the handoff.
...
| A single lived quote or passage worth preserving | Not a tier yet — it is a **source** for `Atelier/10-Sources/` (use `q-capture`) |
| A long raw corpus to mine later | Not a tier — it is **Quarry** material (use `q-export`) |
